...
class Drawable {
  ...
  draw(gl, camera, primitive) {
    ...
    gl.bufferData(gl.ARRAY_BUFFER, this.vertexBufferInfo.data, gl.DYNAMIC_DRAW);
    ...
    gl.bufferData(gl.ARRAY_BUFFER, this.normalBufferInfo.data, gl.DYNAMIC_DRAW);
...
class Mesh extends Drawable {
  constructor(gl, shader, vertices, position = [0,0,0], color = [0,0,0,0]) {
    const vertexInfo = new BufferInfo(gl, vertices, vertices.length/3);
    super(gl, shader, vertexInfo, vertexInfo, position);
    this.color = vec4.create();
    vec4.set(this.color, color[0], color[1], color[2], color[3]);
  }
  ...
  updateHeights(dt, offset) {
    for (let i = 0; i < this.vertexBufferInfo.data.length; i+=3) {
      this.vertexBufferInfo.data[i+1] = Math.sin(this.vertexBufferInfo.data[i] + offset) * Math.cos(this.vertexBufferInfo.data[i+2] + offset);
    }
  }
}
