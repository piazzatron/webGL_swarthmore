...
class Drawable {
  ...
  draw(gl, camera, primitive) {
    ...
    gl.uniformMatrix4fv(this.u_mMatrix, false, modelMatrix);
  ...
  update() {
    ...
    vec3.scale(scaledV, this.velocity, dt);
    vec3.add(this.position, this.position, scaledV);
  }
}
...
class Particle extends Drawable {
  constructor(gl, shader, mass = 1, position = [0,0,0], velocity = [0,0,0], color = [0,0,0,0]) {
    const vertexInfo = new BufferInfo(gl, position, 1);
